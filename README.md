...
- 真实 3D 生成：线上通过 Meshy、离线开发可通过本地 TripoSR sidecar，把同一张方案图转换为统一任务状态和 GLB 产物。
...
- 后端：Node.js、Express
- AI：Google Gemini 图像生成接口、Meshy image-to-3D；本地可选 TripoSR
- 数据库：Supabase
- 测试：Vitest、Supertest
...
- Vercel 只支持 `THREE_D_PROVIDER=meshy`；未配置 `MESHY_API_KEY` 时能力接口会明确返回未就绪，不会静默切换 provider。
- 本地离线 3D 可将 `THREE_D_PROVIDER` 设为 `local`，并按 `.env.example` 配置 `LOCAL_3D_*` / `TRIPOSR_*`；运行脚本见 `scripts/setup-local-3d.ps1` 和 `scripts/start-local-3d.ps1`。
...
- `GET /api/3d-capabilities`：读取当前 3D provider 与可用状态
- `POST /api/generate-3d`：创建真实 3D 任务
...
- 主要线上平台：Vercel（`vercel.json` 会将 Serverless API 路由放在静态回退之前）。线上真实 3D 必须使用 Meshy；`.vercelignore` 会排除本地 Python sidecar、模型缓存和临时产物。
